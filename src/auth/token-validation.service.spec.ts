...
describe('TokenValidationService', () => {
  ...
  it('valida um token valido e retorna o subject', () => {
    const token = jwt.sign({ sub: 'user-123' }, secret, { issuer, algorithm: 'HS256' });
    expect(service.validate(token)).toBe('user-123');
  });

  it('rejeita token com secret errado', () => {
    const token = jwt.sign({ sub: 'user-123' }, 'wrong-secret', { issuer, algorithm: 'HS256' });
    expect(() => service.validate(token)).toThrow();
  });

  it('rejeita token com issuer errado', () => {
    const token = jwt.sign({ sub: 'user-123' }, secret, { issuer: 'wrong-issuer', algorithm: 'HS256' });
    expect(() => service.validate(token)).toThrow();
  });
});
